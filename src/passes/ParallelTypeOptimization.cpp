...
//  )
//
// Here we read the vtable and then test it to see if it has a particular type.
// If vtable types parallel class types, which can be the case for languages
...
//  )
//
// That is, when we find parallel class hierarchies then we can map operations
// from one to the other, saving work.
//

#include "ir/module-utils.h" // XXX ?
...
struct ParallelTypeOptimization : public Pass {
  // XXX
...
TypeMapper::TypeUpdates refinableTypes;
  ...
  void run(Module* module) override {
    ...
    if (!getPassOptions().closedWorld) {
      Fatal() << "ParallelTypeOptimization requires --closed-world";
    }

    // First, compute subtypes.
    SubTypes subTypes(*module);

    // Find parallel hierarchies. We are looking specifically for the vtable
    // pattern: type $A writes to a field a type $vtable.A, and $vtable.A is not
    // used anywhere else in that field.
    //
    // We use static
  ...
  void optimize(Module* module, const SubTypes& subTypes) {
    ...
    class ParallelTypeOptimizationTypeMapper : public TypeMapper {
    public:
      ParallelTypeOptimizationTypeMapper(Module& wasm, const TypeUpdates& mapping)
    ...
    ParallelTypeOptimizationTypeMapper(*module, mapping).map();
...
Pass* createParallelTypeOptimizationPass() { return new ParallelTypeOptimization(); }
