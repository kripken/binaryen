...
Function* EmscriptenGlueGenerator::generateAssignGOTEntriesFunction() {
  ...
  for (Global* g : gotFuncEntries) {
    ...
    if (ex) {
      ...
      auto* relativeBase = builder.makeGlobalGet(tableBase->name, Type::i32);
      auto* add = builder.makeBinary(AddInt32, relativeBase, c);
